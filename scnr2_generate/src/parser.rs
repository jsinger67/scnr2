...
//! The parser is used to parse regex syntax into an high-level intermediate representation (HIR).
...
pub(crate) fn parse_regex(input: &str) -> Result<regex_syntax::hir::Hir> {
    let now = Instant::now();
...
mod tests {
    ...
    fn test_parse_regex_syntax_valid() {
        // Valid regex syntax
        let input = r"\d";
        let hir = parse_regex(input).unwrap();
    ...
    #[test]
    #[should_panic(expected = "Parse(Error { kind: ClassUnclosed")]
    fn test_parse_regex_syntax_invalid() {
        // Invalid regex syntax
        let input = r"^\d{4}-\d{2}-\d{2}$[";
        let _ = parse_regex(input).unwrap();
    }
    ...
    #[test]
    #[should_panic(expected = "Parse(Error { kind: UnsupportedLookAround")]
